...
if __name__ == "__main__":
    ...
    # Pad plaintext to multiple of 8 characters
    if len(plaintext) % 8 != 0:
        plaintext = plaintext.ljust((len(plaintext) // 8 + 1) * 8, ' ')
    
    # Split into 8-character blocks and encrypt each
    cipher_blocks = []
    for i in range(0, len(plaintext), 8):
        block = plaintext[i:i+8]
        cipher_block = DES_encrypt(block, round_keys)
        cipher_blocks.append(cipher_block)
    
    cipher = "".join(cipher_blocks)
    print("Cipher Text:", cipher)

    # Decrypt each 8-character block
    decrypted_blocks = []
    for i in range(0, len(cipher), 8):
        cipher_block = cipher[i:i+8]
        decrypted_block = DES_decrypt(cipher_block, round_keys)
        decrypted_blocks.append(decrypted_block)
    
    recovered = "".join(decrypted_blocks)
    print("Decrypted Text:", recovered.rstrip(' '))  # Remove padding spaces